...
L: Lista
U: Na lista, copia o Nome de Usuário
P: Na lista, copia a Senha
D: Na lista, Exclui
E: Na lista, Edita
S: Pesquisar
Insert: Inserir nova Senha
Tab: Ir para o próximo campo
Shift+Tab: Ir para o campo anterior
...
fn main() -> Result<(), Box<dyn Error>> {
    let passphrase = rpassword::prompt_password("Insira senha mestra: ").unwrap();
...
fn new_section<B: Backend>(f: &mut Frame<B>, state: &mut PassMng, area: Rect) {
    ...
    let title_input = Paragraph::new(state.new_title.to_owned())
        .block(Block::default().title("Conta").borders(Borders::ALL).border_type(BorderType::Rounded))
    ...
    let password_input = Paragraph::new(state.new_password.to_owned())
        .block(Block::default().title("Senha").borders(Borders::ALL).border_type(BorderType::Rounded))
        .style(match state.mode {
    ...
    let submit_btn = Paragraph::new("Submeter")
...
fn list_section<B: Backend>(f: &mut Frame<B>, state: &mut PassMng, area: Rect) {
    ...
    let search_input = Paragraph::new(state.search_txt.to_owned())
        .block(Block::default().title("Buscar").borders(Borders::ALL).border_type(BorderType::Rounded))
        .style(match state.mode {
...
fn delete_popup<B: Backend>(f: &mut Frame<B>, state: &mut PassMng) {
    if let InputMode::Delete = state.mode {
        ...
        let text = Paragraph::new("Você tem certeza?")
            .style(Style::default().fg(Color::Red))
            .alignment(Alignment::Center);
        f.render_widget(text, chunk[0]);

        let keys_desc = Paragraph::new("Pressione (Y) para Sim e (N) para Não")
            .alignment(Alignment::Center);
